...
const generateGrandReport = async (startDate, endDate) => {
  try {
    ...
    const pipeline = [
      ...
      // Group by resolved project id + name and user role
      {
        $group: {
          ...
          descriptions: { $push: '$tasks.description' }
        }
      },
      // Concatenate descriptions per role
      {
        $addFields: {
        ...
        $group: {
          ...
          roles: {
            $push: {
              role: '$_id.role',
              hours: '$totalHours',
              descriptions: '$concatenatedDescriptions'
            }
          },
    ...
    const projects = projectData.map(project => {
      ...
      // Fill in role-specific hours and descriptions
      project.roles.forEach(roleData => {
        if (roleData.role in projectObj) {
          projectObj[roleData.role] = roleData.hours;
          projectObj[`${roleData.role}_descriptions`] = roleData.descriptions || '';
        }
      });
